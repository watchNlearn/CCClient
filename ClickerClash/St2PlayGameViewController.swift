//
//  St2PlayGameViewController.swift
//  ClickerClash
//
   ...
   // @IBOutlet weak var bannerView: GADBannerView!
            ...
            ref.child("tournaments").child("standard").child("st2").child("usersPlaying").observeSingleEvent(of: .value, with: { (snapshot) in
                if snapshot.hasChild(self.username!){//updating the users score
                    ...
                    if currentCountHere >= clashHighScoreStart {
                        ...
                        let ref = Database.database().reference()
                        let uid = Auth.auth().currentUser!.uid
                        ref.child("tournaments").child("standard").child("st2").child("usersPlaying").child(self.username!).setValue(currentCountHere)
                        print(currentCountHere)
    ...
    @objc func counter2() {
        ...
        if timeLeft >= 0 {
            ...
            self.s2TimeLeft.text = timeString(time: timeCount)
        }
        else {
            self.buttonOutlet.isEnabled = false
            self.buttonOutlet.setTitleColor(UIColor .darkGray, for: .disabled)
